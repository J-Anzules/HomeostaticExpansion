...
# ggsave("C:/Laptop Backups/HomestaticExpansionProject/Plots/HomeoPlots/Young/ThymusWeight.png",
#        width = 5.5,
#        height = 4)
```
# Spleen Weight
...
#         text = element_text(size=AxisTickSize))+
#   stat_summary(aes(group=Genotype, color = Genotype), fun=mean, geom="line")
# ggsave("C:/Laptop Backups/HomestaticExpansionProject/Plots/HomeoPlots/Young/SpleenWeight.png",
#        width = 5.5,
#        height = 4)
```
...
ActivationDataSpleen = subset(ActivationData, Organ == "Spleen")
#Changing total cell count to 10**6
ActivationDataSpleen$NaiveCT = ActivationDataSpleen$NaiveCT * 10**6
ggplot(ActivationDataSpleen, aes(intage, NaiveCT, shape = Genotype)) + 
  geom_point(position = position_dodge(1), size = 4)+
  scale_x_continuous(breaks = seq(0,18,2))+
  scale_y_continuous(breaks = seq(0,max(ActivationDataSpleen$NaiveCT), length.out = 5), labels =label_scientific(digits = 2) )+
  labs(titles = "Naive T Cell Total Count", x = "Age in Days", y = "CD4+TCRbeta_high-CD69-Foxp3-")+
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 2),
                 legend.key = element_rect(fill = "white", colour = "black"),
                 legend.background = (element_rect(colour= "black", fill = "white")),
        axis.title.x = element_text(face="bold", colour="black", size=xAxisTextSize),
        axis.title.y = element_text(face = "bold", colour = "black", angle = 90, size = xAxisTextSize),
        plot.title = element_text(lineheight=.8, face="bold", size = titleAxisTextSize),
        axis.ticks.length=unit(.25, "cm"),
        text = element_text(size=AxisTickSize))+
  stat_summary(aes(group=Genotype, color = Genotype), fun=mean, geom="line")
# day18 = subset(ActivationDataSpleen, intage == 18 )
# day18wt = subset(ActivationDataSpleen, Genotype == "WT")
# day18ko = subset(ActivationDataSpleen, Genotype == "KO")
# t.test(day18wt$NaiveCT, day18ko$NaiveCT)
...
TregData = subset(popSpln, intage > 0)
...
ggplot(TregData, aes(intage, X4TregCT, shape = Genotype)) + 
  ...
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 2),
        ...
        text = element_text(size=AxisTickSize))+
  stat_summary(aes(group=Genotype, color = Genotype), fun=mean, geom="line")
ggsave("C:/Laptop Backups/HomestaticExpansionProject/Plots/HomeoPlots/Young/Treg_Count.pdf",
       width = 5.5,
       height = 4)
...
ggplot(TregData, aes(intage, X4TregRatio, shape = Genotype)) + 
  ...
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 2),
        ...
        text = element_text(size=AxisTickSize))+
  stat_summary(aes(group=Genotype, color = Genotype), fun=mean, geom="line")
ggsave("C:/Laptop Backups/HomestaticExpansionProject/Plots/HomeoPlots/Young/Treg_Pct.png",
       width = 5.5,
       height = 4)
```
...
#P values in those populations
WTTregProlD14 = subset(longpopTregs2,variable == "X4TregProlCT" & Age == 18 )
KOTregProlD14 = subset(longpopTregsKO, variable == "X4TregProlCT" & Age == 18 )

t.test(WTTregProlD14$value, KOTregProlD14$value)
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
#             Naive T cell and sub pops                 #
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#


#Preparing the data
popActT = subset(popActivKO, select = c("Age", "NaiveCT", "ThymicNaive"))
prolActT = subset(WTProl, select = c("Age", "NaiveProlCT"))
AcTpops = merge(popActT, prolActT)

longAcTpops = gather(AcTpops, variable, value, -Age)
...
View(longAcTpops)
ggplot(longAcTpops, aes(x = Age, y = value, color = variable)) + 
  geom_point(position = position_dodge(1), size = 2)+
  stat_summary(aes(group=variable, color = variable), fun=mean, geom="line")+
  labs(titles = "Different KO Treg sources", x = "Age in Days", y = "Treg counts")+
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 2),
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
#              ActT T cell and sub pops                 #
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#


#Activated T cell Data now
...
plot(pop$OrganWeight, pop$CD4CT, xlim = c(0, 0.3),
     main = "Total CD4 CT vs. Thymus Weight",
     xlab = "Thymus weigth (grams)",
     ylab = "CD4 Counts (10^6)",
     pch = 16, cex = 1.3, col = "blue")
abline(lm(pop$CD4CT~pop$OrganWeight))

cor(pop$CD4CT,pop$OrganWeight, use = "na.or.complete")

#Treg
plot(pop$OrganWeight, pop$X4TregCT, xlim = c(0, 0.3),
     main = "Total CD4 CT vs. Thymus Weight",
     xlab = "Thymus weigth (grams)",
     ylab = "CD4 Counts (10^6)",
     pch = 16, cex = 1.3, col = "blue")
abline(lm(pop$X4TregCT~pop$OrganWeight))

cor(pop$X4TregCT,pop$OrganWeight, use = "na.or.complete")
```
